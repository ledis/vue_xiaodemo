...
<script>
	export default{
		...
		methods:{
			fetchData(id){
				var _this=this;
				this.$axios.get('src/data/article.data').then(function(res){
...
<style scoped>
.nav{width:100%; position:fixed;top:0;left:0;height:0.6rem; background:#fff; border-bottom:1px solid #e8eaec; z-index:99;}
.nav ul{width:6.4rem;height:0.6rem;margin:0 auto;}
.nav ul li{width:0.6rem;height:0.6rem; background:url(../../assets/img/history.png) no-repeat center center; background-size:0.29rem 0.31rem;}
...
.fx i{width:0.33rem;height:0.08rem;display:block; margin:0 auto; padding-top:0.10rem;}
.fx i img{width:100%;}
</style>